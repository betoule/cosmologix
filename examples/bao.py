...
for dist_type, distfunc, zscale, label in [('DV_over_rd', distances.dV, (2./3), r'$\frac{D_V}{r_d z^{2/3}}$'),
    ...
    ey = error[goods] 
    l = ax1.errorbar(zdata, ydata/zdata**(zscale), ey/zdata**(zscale), marker='o', ls='None')
    ax1.plot(z, distfunc(aparams, z)/(aparams['rd']*z**(zscale)), color=l[0].get_color(), label=label)
    ax1.plot(z, distfunc(Planck18, z)/(rd(Planck18)*z**(zscale)), color=l[0].get_color(), ls='--')
    ax2.errorbar(desiu.redshifts[goods], res[goods], error[goods], ls='None', marker='s')
ax2.set_xlabel(r'$z$')
