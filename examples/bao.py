...
ax2.axhline(0, color="k", label=rf"$\Omega_{{bc}} = {Om:.3f} \pm {eOm:.3f}$""\n"
            rf"$r_d h = {rdh:.1f} \pm {erdh:.1f}$")
ax1.legend(loc="lower right", frameon=False)
ax2.legend(loc="lower right", frameon=False)

# Let's split the DESIDR2 prior by sub-samples
plt.figure('Fig 7.')
def subprior(index):
    mask = np.array(index)
...
prior = subprior([0])
bf = fitter.fit([prior], fixed={**fixed, 'rd':151.})
...
for label, index in subsamples.items():
    ...
    contour = contours.frequentist_contour_2d(
        [prior],
        grid={'rd': [hrdtord(85), hrdtord(115), 100],
              'Omega_bc': [0.15, 0.55, 100]},
        fixed=fixed
    )
