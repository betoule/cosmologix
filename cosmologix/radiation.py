...
def Omega_n_rel(params: Dict[str, float]) -> float:
    ...
    omrnomass = omr * (params["Neff"] - 3.046 / 3.)  # Note: This might need adjustment if massless neutrinos are considered
    return omrnomass
...
def nu_rho_interp(am: jnp.ndarray) -> jnp.ndarray:
    ...
    rhonu = rhonu.at[indexhigh].set(1.5 / Constants.const * (
        Constants.zeta3 * am[indexhigh] + 0.75 * 15 * Constants.zeta5 / am[indexhigh]))
    return rhonu
